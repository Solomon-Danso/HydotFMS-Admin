...
formData.append("SubTitle",SubTitle);
    ...
    const response = await fetch(apiServer+"CreateSlider", {
      method: "POST",
      headers: {
...
const handleEditAdmin = async (SliderID) => {
...
formData.append("SliderID",SliderID);
    ...
    const response = await fetch(apiServer+"UpdateSlider", {
      method: "POST",
      headers: {
        'UserId': userInfo.UserId,         
        'SessionId': userInfo.SessionId    
...
const handleDeleteAdmin = async (id) => {
  ...
  formData.append("AdminId",userInfo.UserId)
      ...
      const response = await fetch(apiServer+"DeletedSlider", {
        method: "POST",
        headers: {
          'UserId': userInfo.UserId,         
          'SessionId': userInfo.SessionId    
...
const menuItems = [
  {
    ...
    type: "function",
    onClick: (SliderID) => {
      handleDeleteAdmin(SliderID); // Assuming this function is defined in your component
    },
    columnNames: ['SliderID'] // Specify the column name for the ID here
  }
 ...
 const exploreGrid = [
  ...
  { accessorKey: "SliderID", header: "Slider ID" },
      ...
      <div className="card" style={{ backgroundColor: localStorage.getItem("themeMode") === "Light" ? "#26293C" : "white", padding: "2rem" }}>
       ...
       <Stepper activeStep={activeStep} alternativeLabel sx={{ padding: '2rem 0' }}>
        ...
        <Box sx={{ padding: 3 }}>
          {activeStep === 0 && (
            <div>
              <div className="sec-title" style={{ color: localStorage.getItem("colorMode"), padding: "2rem" }}>Add Slides</div>
              <Selector
