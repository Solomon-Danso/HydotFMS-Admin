...
useEffect(()=>{
  ...
  fetch(apiServer+"ViewAllRentACar",{
    method: "POST",
        headers: {
...
formData.append("FuelType",FuelType);
    ...
    const response = await fetch(apiServer+"CreateRentACar", {
      method: "POST",
      headers: {
        'UserId': userInfo.UserId,         
        'SessionId': userInfo.SessionId    
...
formData.append("RentACarID",ExploreID);
    ...
    const response = await fetch(apiServer+"UpdateRentACar", {
      method: "POST",
      headers: {
        'UserId': userInfo.UserId,         
        'SessionId': userInfo.SessionId    
...
const handleDeleteAdmin = async (id) => {
  ...
  const formData = new FormData()
  formData.append("RentACarID", id) 
  formData.append("AdminId",userInfo.UserId)

  
      const response = await fetch(apiServer+"DeletedRentACar", {
        method: "POST",
        headers: {
          'UserId': userInfo.UserId,         
          'SessionId': userInfo.SessionId    
...
const menuItems = [
  {
    ...
    text: "Add Sources",
    type: "navigate",
    path: `/main/rentACar/:RentACarID`, // Placeholder for the dynamic segment
  },
  {
    icon: <FaCar />,
    text: "Add Specifications",
    type: "navigate",
    path: `/main/rentACar/specs/:RentACarID`, // Placeholder for the dynamic segment
  },
  {
    icon: <MdDelete />,
    text: "Delete Details",
    ...
    columnNames: ['RentACarID'] // Specify the column name for the ID here
  },
...
const handleReset = () => {
  ...
  return (
    <div>
      <Header category="Website Configuration" title="Rent A Car" />
      ...
      <div className="card" style={{ backgroundColor: localStorage.getItem("themeMode") === "Light" ? "#26293C" : "white", padding: "2rem" }}>
       ...
       <Stepper activeStep={activeStep} alternativeLabel sx={{ padding: '2rem 0' }}>
        ...
        <Box sx={{ padding: 3 }}>
          {activeStep === 0 && (
            <div>
              <div className="sec-title" style={{ color: localStorage.getItem("colorMode"), padding: "2rem" }}>Add RentACar</div>
              <Selector
